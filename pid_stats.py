...
def parse_options():
    try:
        ...
        opts, args = getopt.getopt(sys.argv[1:], "shtdSp:w:mq", long_options)
...
def help():
    help_msg = 'Usage: ps_mem [OPTION]...\n' \
        ...
        '  -m, --mb                    Show output in megabytes rather than human-readable\n' \
...
def print_quiet_memory_swap_usage(sorted_cmds, shareds, count, total, swaps, total_swap,
                       in_mb):
    units = None;
    unit = ""
    if in_mb:
    ...
    for cmd in sorted_cmds:
        ...
        output_data = (human(cmd[1],units=units), unit, human(swaps[cmd[0]],units=units), unit)
        sys.stdout.write("%s %s %s %s\n" % output_data)

def print_memory_usage(sorted_cmds, shareds, count, total, swaps, total_swap,
                       show_swap, in_mb, quiet):
    units = None;
    if in_mb:
        units = 1024.0 * 1024.0;
    for cmd in sorted_cmds:
        ...
        output_data = (human(cmd[1]-shareds[cmd[0]],units=units),
                       human(shareds[cmd[0]],units=units), human(cmd[1],units=units))
        if show_swap:
            output_string += "   %9s"
            output_data += (human(swaps[cmd[0]],units=units),)
        output_string += "\t%s\n"
    ...
    if have_swap_pss and show_swap:  # kernel will have_pss
        sys.stdout.write("%s\n%s%9s%s%9s\n%s\n" %
                         ("-" * 45, " " * 24, human(total,units=units), " " * 3,
                          human(total_swap,units=units), "=" * 45))
    elif have_pss:
        sys.stdout.write("%s\n%s%9s\n%s\n" %
                         ("-" * 33, " " * 24, human(total,units=units), "=" * 33))
...
def main():
    ...
    show_swap, in_mb, quiet = parse_options()
    ...
    if watch is not None:
        ...
        sorted_cmds, shareds, count, total, swaps, total_swap = \
            get_memory_usage(pids_to_show, split_args,
                             discriminate_by_pid)
        if quiet:
            print_quiet_memory_swap_usage(sorted_cmds, shareds, count, total, swaps, total_swap,
                       in_mb)
        elif only_total and show_swap and have_swap_pss:
            sys.stdout.write(human(total_swap, units=1)+'\n')
        elif only_total and not show_swap and have_pss:
            sys.stdout.write(human(total, units=1)+'\n')
        elif not only_total:
            print_memory_usage(sorted_cmds, shareds, count, total, swaps,
                               total_swap, show_swap, in_mb, quiet)
